# 艺培续费增长助手 MVP

[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/fucunzhao/art-training-renewal-assistant)

面向艺术培训机构的续费增长后台原型，支持登录、学员录入、流失风险评分、家长沟通话术生成，以及从本地知识库文件中自动识别学员信息。

## 功能

- 后台登录：默认账号 `admin`，密码 `123456`
- 账号注册：支持新用户注册，内置 5 个测试账号
- 通知服务：站内消息中心、高风险学员提醒、企业微信机器人 webhook 预留
- 学员手动录入：课程、老师、课时、缺勤、家长互动、续费金额、成长证据
- 风险看板：自动计算高风险、快课消、家长沉默学员
- 续费话术：按温和、直接、高客单三种语气生成沟通内容
- AI 录入：后台可上传 `.txt`、`.md`、`.json`、`.csv` 文件，也可扫描 `knowledge_base` 文件夹，抽取候选学员并确认导入
- 教务排课与课消：安排课程、老师、教室和学员，自动检测冲突，完成课节后扣减学员课时

## 本地运行

```bash
npm start
```

然后打开：

```text
http://127.0.0.1:4173/
```

## 配置

可通过环境变量覆盖默认配置：

```bash
PORT=4173
ADMIN_USER=admin
ADMIN_PASS=123456
WECHAT_WORK_BOT_URL=
```

## 知识库文件

把学员记录放入 `knowledge_base` 文件夹。示例：

```text
学员姓名：孙若晴
课程：少儿创意美术进阶
负责老师：唐老师
剩余课时：2
预计课消天数：5
缺勤率：12
家长回复次数：0
作业缺交次数：1
续费金额：4980
最近联系：9天前
成长证据：构图开始有主次，能主动安排主体和背景关系。
```

## 部署说明

这是一个 Node 后台项目，不是纯静态网站。GitHub Pages 只能托管静态页面，不能运行登录、API、本地 JSON 写入和知识库扫描。

推荐部署方式：

- GitHub：托管代码仓库
- Zeabur、Railway、Fly.io、VPS：运行 Node 服务
- Docker：仓库已包含 `Dockerfile`

生产环境还需要替换本地 JSON 存储，改为数据库，并把默认密码改成强密码。

## 测试账号

```text
admin / zzdh6886 / 校长
frontdesk01 / test1234 / 前台
teacher01 / test1234 / 老师
teacher02 / test1234 / 老师
manager01 / test1234 / 校长
```

## 通知服务

MVP 版本支持站内通知和高风险学员提醒。登录后点击右上角“消息”，可以生成高风险提醒、查看未读消息、标记已读。

如果需要同步推送到企业微信群机器人，配置：

```text
WECHAT_WORK_BOT_URL=你的企业微信群机器人 webhook
```

## Zeabur 部署

如果 Render 无法打开，可以使用 Zeabur：

1. 打开 Zeabur 并登录
2. 新建 Project
3. 选择 GitHub 仓库 `fucunzhao/art-training-renewal-assistant`
4. 服务类型选择 Node.js 或 Docker
5. 设置环境变量：

```text
ADMIN_USER=admin
ADMIN_PASS=你自己的强密码
HOST=0.0.0.0
```

Zeabur 会自动读取 `package.json` 的 `start` 命令，或使用仓库里的 `Dockerfile`。

## Docker 部署

```bash
docker build -t art-training-renewal-assistant .
docker run -p 4173:4173 -e ADMIN_USER=admin -e ADMIN_PASS=your-password art-training-renewal-assistant
```
